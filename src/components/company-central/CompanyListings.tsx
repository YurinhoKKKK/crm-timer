...
// Uma listagem está PENDENTE (precisa de reajuste) quando o último evento é um
// pedido de ajuste ou uma contestação DO CLIENTE — a mesma regra da fila.
function isPending(events: ListingValidationItem[]): boolean {
  const latest = events[events.length - 1];
  return (
    !!latest &&
    latest.authorType === "cliente" &&
    (latest.event === "ajuste_solicitado" || latest.event === "contestado")
  );
}
        ...
        <ul className="space-y-2">
          {visible.map((r) => (
            ...
            >
              ...
              <ValidationTimeline events={validations[r.id] ?? []} />
              {isPending(validations[r.id] ?? []) && (
